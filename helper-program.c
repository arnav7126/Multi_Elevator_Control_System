...
int main(int argc, char* argv[]) {
  ...
  // Ensure all message queues and shared memory are removed properly
  if (msgctl(msgId, IPC_RMID, NULL) == -1) {
    perror("Error removing main message queue");
  }
  
  for (int i = 0; i < solverCount; i++) {
    if (msgctl(solverInfo[i].msgId, IPC_RMID, NULL) == -1) {
      printf("Error removing solver %d message queue\n", i);
      perror(NULL);
    }
  }
  ...
  if (shmdt(mainShmPtr) == -1) {
    perror("Error detaching shared memory in helper");
  }
  
  if (shmctl(shmId, IPC_RMID, NULL) == -1) {
    perror("Error removing shared memory segment in helper");
  }
